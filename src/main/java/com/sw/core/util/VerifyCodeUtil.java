...
public class VerifyCodeUtil {
     ...
     * @return 图片缓存对象
     */
    public static BufferedImage createImageCode(int type, int length, String excludeString, int width, int height,
                                                int interLine, boolean randomLocation, Color backColor,
                                                Color foreColor, Color lineColor, HttpServletRequest request){
        String textCode = generateTextCode(type, length, excludeString);
        HttpSession session = request.getSession();
        session.setAttribute("randomCode", textCode);
