(function ($) {
    ...
    BootstrapTable.prototype.initData = function () {
        ...
        if (that.options.treeView && this.data.length > 0) {
            ...
            var roots = $.grep(this.data, function (row, index) {
                return row.menuLevel == that.options.treeRootLevel;
            });
            ...
            $.each(this.header.fields, function (j, field) {
                ...
                if (column.checkbox || column.radio) {
                    ...
                    if (that.options.treeView && column.field == that.options.treeField) {
                        var indent = item.menuLevel == that.options.Level ? '' : sprintf('<span style="margin-left: %spx;"></span>', (item.menuLevel - that.options.treeRootLevel) * 15);
    ...
    $.extend($.fn.bootstrapTable.defaults, {
        treeView: true,//treeView视图
        treeField: "name",//treeView视图字段
        treeId: "id",
        treeRootLevel: 0,//根节点序号
        treeCollapseAll: true,//是否全部折叠
